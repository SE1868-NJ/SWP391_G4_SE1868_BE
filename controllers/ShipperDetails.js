...
//     const checkSql =
//       "SELECT * FROM Shippers WHERE ShipperID = ? AND Status = 'PendingRegister'";
...
//           success: false,
//           message:
//             "Không tìm thấy shipper hoặc shipper không trong trạng thái chờ duyệt",
//         });
//       }
...
//       const shipperName = checkResult[0].FullName;
//       const updateSql =
//         "UPDATE Shippers SET Status = 'Active' WHERE ShipperID = ?";
...
const approveShipper = (req, res) => {
  ...
  try {
    ...
    db.query(checkSql, [shipperId], (checkErr, checkResult) => {
      ...
      const shipperName = checkResult[0].FullName;
      const updateSql =
        "UPDATE Shippers SET Status = 'PendingDepositEscrow' WHERE ShipperID = ?";
      db.query(updateSql, [shipperId], (updateErr, updateResult) => {
        if (updateErr) {
          return res.status(500).json({
            success: false,
        ...
        res.json({
          success: true,
          message: "Đã duyệt shipper thành công",
          data: { shipperId, newStatus: "Active" },
        });
      });
    });
  } catch (error) {

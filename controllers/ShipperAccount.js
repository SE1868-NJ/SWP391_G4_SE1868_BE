...
const depositToWallet = async (req, res) => {
  try {
    ...
    const [shipperRows] = await new Promise((resolve, reject) =>
      db.query('SELECT ShipperID FROM Shippers WHERE ShipperID = ?', [shipperId], (err, results) =>
    ...
    const extraData = Buffer.from(JSON.stringify({ shipperId })).toString('base64');
    ...
    const signature = generateMoMoSignature(momoParams);
    ...
    res.status(500).json({
      success: false,
      message: 'Lỗi server khi khởi tạo thanh toán',
    });
  }
};
...
const generateMoMoSignature = (params) => {
...
const handleMoMoIPN = async (req, res) => {
  try {
    ...
    const computedSignature = generateMoMoSignature(momoParams);
    ...
    if (resultCode === 0) {
      const { shipperId } = JSON.parse(Buffer.from(extraData, 'base64').toString());

      await new Promise((resolve, reject) =>
        db.query(
          'INSERT INTO EWallet (ShipperID, Balance) VALUES (?, ?) ON DUPLICATE KEY UPDATE Balance = Balance + ?',
          [shipperId, amount, amount],
          (err) => (err ? reject(err) : resolve())
        )
      );

      console.log(`Wallet updated for shipper ${shipperId} with amount ${amount}`);
    }
